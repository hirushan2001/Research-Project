...
const FUTURE_FEATURES: FutureFeature[] = [
  {
    id: 'feat-1',
    title: 'Ripeness Zoning',
    description: 'Determine spatial distribution of green-to-orange ripeness zones across Solo/Hawaiian papaya peels.',
    icon: 'sparkles'
  },
  {
    id: 'feat-2',
    title: 'Disease Spot Detection',
    description: 'Track density and spread ratio of anthracnose or black spot lesions to evaluate rejection thresholds.',
    icon: 'eye'
  }
];

export const PapayaGrading: React.FC = () => {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      const result = await predictPapaya(selectedFile);
      ...
      setError(
        ...
        'Failed to connect to the papaya backend services. Please verify the FastAPI application is running.'
      );
    } finally {
      setIsLoading(false);
    }
      ...
      <section className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-2xl md:text-3xl font-extrabold text-slate-800 dark:text-white">
            Papaya Quality Assessment
          </h2>
        </div>
        <p className="text-sm text-slate-550 dark:text-slate-400 font-sans max-w-2xl leading-relaxed">
          Upload a clear photograph of a papaya specimen. The model will analyze ripeness levels, disease lesions, and physical surface defects, outputting a graded quality report (G1 - G4).
        </p>
      </section>
          ...
          {prediction ? (
            ...
            <div className="h-full min-h-[300px] flex flex-col items-center justify-center text-center p-8 rounded-3xl border border-slate-200/50 dark:border-slate-800/45 bg-white/40 dark:bg-slate-900/10 glass-panel">
              ...
              <p className="mt-1.5 text-xs text-slate-450 dark:text-slate-550 max-w-[280px]">
                Please upload an image file of a papaya specimen and trigger the grading analysis model to view results.
              </p>
            </div>
          )}
        </div>
