    ...
    <div className="space-y-10 fade-in py-6">
      ...
      <section className="space-y-2">
        ...
        <p className="text-sm text-slate-550 dark:text-slate-400 font-sans max-w-2xl leading-relaxed">
          Automated classification model node to assess papaya maturity stages, surface lesions, and physical skin damage.
        </p>
      </section>
      ...
      <section className="flex flex-col items-center justify-center text-center p-12 rounded-3xl border border-slate-200/50 dark:border-slate-800/40 bg-white/40 dark:bg-slate-900/10 glass-panel max-w-2xl mx-auto mt-6">
        ...
        <p className="mt-3 text-sm text-slate-500 dark:text-slate-400 leading-relaxed max-w-md">
          This workspace is pre-configured and isolated for the Papaya Grading module. Teammates can customize this page and integrate their specific YOLOv8 inference pipeline inside <code className="px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-xs font-mono">PapayaGrading.tsx</code>.
        </p>
        ...
        <div className="mt-8 p-4 w-full rounded-2xl border border-slate-200/60 dark:border-slate-800/40 bg-slate-50/50 dark:bg-slate-950/40 text-left text-xs space-y-2">
          <div className="flex justify-between">
            ...
            <span className="font-semibold text-slate-700 dark:text-slate-300">Papaya Developer</span>
          </div>
          <div className="flex justify-between">
            <span className="text-slate-400 dark:text-slate-500">Backend Endpoint</span>
            <span className="font-mono text-emerald-650 dark:text-emerald-450">POST /api/papaya/predict</span>
          </div>
          <div className="flex justify-between">
            <span className="text-slate-400 dark:text-slate-500">API Key Config</span>
            <span className="font-mono text-amber-605 dark:text-amber-405">PAPAYA_ROBOFLOW_API_KEY</span>
          </div>
        </div>
      </section>
