...
logger = logging.getLogger("banana-grading")

HAS_INFERENCE_SDK = True

GRADE_MESSAGES = {
    "G1": "Excellent Quality Banana (Optimal yellow, firm texture, no bruises or black spots)",
    "G2": "Good Quality Banana (Minor tip greenness or tiny spots, great for retail)",
    "G3": "Average Quality Banana (Slightly overripe, moderate brown spotting, ideal for baking)",
    "G4": "Poor Quality Banana (Severely bruised, split skin, or completely black/mushy)"
}
...
def predict_banana_grade(image_path: str, filename: str = "") -> Dict[str, Any]:
    """
    ...
    has_api_key = (
        settings.BANANA_ROBOFLOW_API_KEY 
        and settings.BANANA_ROBOFLOW_API_KEY.strip() != "" 
        and settings.BANANA_ROBOFLOW_API_KEY != "your_banana_api_key_here"
    )
    
    if not has_api_key:
        logger.warning("Banana Roboflow API Key is missing or default. Falling back to simulated mock prediction.")
    ...
    url = f"https://serverless.roboflow.com/banana-grading-system/1?api_key={settings.BANANA_ROBOFLOW_API_KEY}"
    headers = {"Content-Type": "application/x-www-form-urlencoded"}

    logger.info("Calling Roboflow Serverless Inference API (banana-grading-system/1)...")
