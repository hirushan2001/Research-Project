...
logger = logging.getLogger("guava-grading")

HAS_INFERENCE_SDK = True

GRADE_MESSAGES = {
    "G1": "Excellent Quality Guava (Optimal maturity, firm skin, no pest markings)",
    "G2": "Good Quality Guava (Slight skin color variation, suitable for retail)",
    "G3": "Average Quality Guava (Mild bruising, best for local jams or juices)",
    "G4": "Poor Quality Guava (Over-ripe, insect damage, or severe bruising)"
}
...
def predict_guava_grade(image_path: str, filename: str = "") -> Dict[str, Any]:
    """
    ...
    has_api_key = (
        settings.GUAVA_ROBOFLOW_API_KEY 
        and settings.GUAVA_ROBOFLOW_API_KEY.strip() != "" 
        and settings.GUAVA_ROBOFLOW_API_KEY != "your_guava_api_key_here"
    )
    
    if not has_api_key:
        logger.warning("Guava Roboflow API Key is missing or default. Falling back to simulated mock prediction.")
    ...
    # This is a placeholder model endpoint that the guava teammate can edit later!
    url = f"https://serverless.roboflow.com/guava-grading-system/1?api_key={settings.GUAVA_ROBOFLOW_API_KEY}"
    headers = {"Content-Type": "application/x-www-form-urlencoded"}

    logger.info("Calling Roboflow Serverless Inference API (guava-grading-system/1)...")
