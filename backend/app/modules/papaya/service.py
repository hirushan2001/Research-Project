...
logger = logging.getLogger("papaya-grading")

HAS_INFERENCE_SDK = True

GRADE_MESSAGES = {
    "G1": "Excellent Quality Papaya (Slight yellow coloration, firm skin, no soft spots or mold)",
    "G2": "Good Quality Papaya (Nice yellow-orange skin, standard sweetness profile, minor blemishes)",
    "G3": "Average Quality Papaya (Ripe/soft texture, moderate spot markings, consume immediately)",
    "G4": "Poor Quality Papaya (Over-ripe, fermented, deep black spots, or mold growth)"
}
...
def predict_papaya_grade(image_path: str, filename: str = "") -> Dict[str, Any]:
    """
    ...
    has_api_key = (
        settings.PAPAYA_ROBOFLOW_API_KEY 
        and settings.PAPAYA_ROBOFLOW_API_KEY.strip() != "" 
        and settings.PAPAYA_ROBOFLOW_API_KEY != "your_papaya_api_key_here"
    )
    
    if not has_api_key:
        logger.warning("Papaya Roboflow API Key is missing or default. Falling back to simulated mock prediction.")
    ...
    url = f"https://serverless.roboflow.com/papaya-grading-system/1?api_key={settings.PAPAYA_ROBOFLOW_API_KEY}"
    headers = {"Content-Type": "application/x-www-form-urlencoded"}

    logger.info("Calling Roboflow Serverless Inference API (papaya-grading-system/1)...")
